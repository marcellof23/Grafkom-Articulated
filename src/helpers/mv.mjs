...
function arrayToMat4(matrix) {
  var m = [];
  var cnt = 0;
  for (var i = 0; i < 4; i++) {
    var arr = [];
    for (var j = 0; j < 4; j++) {
      arr.push(matrix[cnt + j]);
    }
    m.push(vec4(arr));
    cnt += 4;
  }
...
function lookAt(eye, at, up) {
  ...
  var result = matrix4(vec4(n, -dot(n, eye)), vec4(u, -dot(u, eye)), vec4(v, -dot(v, eye)), vec4());
...
function transposes(m) {
  if (!m.matrix) {
    return "transposes(): trying to transposes a non-matrix";
  }
...
var sizeof = {
  ...
  matrix4: new Float32Array(flatten(matrix4())).byteLength,
};
